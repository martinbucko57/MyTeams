//
//  MatchListCoordinator.swift
//  MyTeams
//
//  Created by Martin Bucko on 05/01/2022.
//

import UIKit

protocol MatchListCoordinatorType {
    func start() -> UIViewController
    func showActionSheet(leagueList: [League], completion: @escaping (League) -> Void)
    func showMatchDetail(for fixtureId: Int)
}

class MatchListCoordinator: MatchListCoordinatorType {

    private var parentCoordinator: TeamDetailCoordinatorType
    private weak var currentController: MatchListViewController?
    
    private let team: Team

    init(parentCoordinator: TeamDetailCoordinatorType, team: Team) {
        self.parentCoordinator = parentCoordinator
        self.team = team
    }
    
    func start() -> UIViewController {
        let viewModel = MatchListViewModel(team: team)
        viewModel.coordinator = self
        let controller = MatchListViewController.instantiate()
        controller.viewModel = viewModel
        currentController = controller
        return controller
    }
    
    func showActionSheet(leagueList: [League], completion: @escaping (League) -> Void) {
        let alert = UIAlertController(title: "Select league", message: nil, preferredStyle: .actionSheet)
        leagueList
            .forEach { league in
            alert.addAction(UIAlertAction(title: league.name, style: .default, handler: { _ in
                completion(league)
            }))
        }
        currentController?.present(alert, animated: true)
    }
    
    func showMatchDetail(for fixtureId: Int) {
        parentCoordinator.showMatchDetail(for: fixtureId)
    }
}
